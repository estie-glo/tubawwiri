...
foreach ($files as $file) {
    ...
    foreach ($fieldMatches as $m) {
        $type = $m[1];
        $name = $m[2];
        $labelField = $m[3] ?? ucfirst($name);
        $fields[] = ['type' => $type, 'name' => $name, 'label' => $labelField];
    }
...
LATEX;
if ($icon) {
    $tex .= "\\textbf{Icône du menu :} \\texttt{$icon}\\\\\n";
}
if ($group) {
    $tex .= "\\textbf{Groupe de menu :} $group\\\\\n";
}
$tex .= <<<LATEX
...
\item Dans le menu latéral, cliquez sur l'icône correspondant à la ressource « $label ».
