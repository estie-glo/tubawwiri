...
@php
    ...
    $photoUrl = $program->cover_image ? asset('storage/' . $program->cover_image) : asset('images/community/village.jpg');
    $wide = $program->slug === 'parents-tbw';
@endphp

@if ($wide)
    <a href="{{ route('programs.show', [app()->getLocale(), $program->slug]) }}"
       class="reveal group relative flex items-center min-h-[220px] bg-white rounded-2xl overflow-hidden hover-lift">
        <img src="{{ $photoUrl }}" alt="" class="absolute inset-0 w-full h-full object-cover transition duration-500 group-hover:scale-105">
        <div class="absolute inset-0 bg-gradient-to-r from-white via-white/85 md:via-white/60 to-transparent"></div>
        <div class="relative z-10 flex items-center gap-4 p-8 max-w-sm">
            <div class="w-16 h-16 rounded-full bg-white border-2 border-[#C99A3E] flex items-center justify-center shrink-0 shadow-sm">
                <svg class="w-7 h-7 text-[#123D2E]" viewBox="0 0 24 24" fill="none" aria-hidden="true">{!! $themeIcons[$program->slug] !!}</svg>
            </div>
            <div>
                <h3 class="font-display text-xl font-semibold text-[#123D2E]">{{ localized($program, 'title') }}</h3>
                <p class="text-sm text-[#6F6759] mt-1.5 leading-relaxed">{{ localized($program, 'summary') }}</p>
            </div>
        </div>
    </a>
@else
    <a href="{{ route('programs.show', [app()->getLocale(), $program->slug]) }}"
       class="reveal group block bg-white rounded-2xl overflow-hidden hover-lift" style="transition-delay: {{ ($index % 3) * 80 }}ms">
        <div class="relative h-40 overflow-hidden">
            <img src="{{ $photoUrl }}" alt="" class="absolute inset-0 w-full h-full object-cover transition duration-500 group-hover:scale-105">
        </div>
        <div class="relative px-6 pb-7 text-center">
            <div class="w-16 h-16 rounded-full bg-white border-2 border-[#C99A3E] flex items-center justify-center mx-auto -mt-8 shadow-sm relative z-10">
                <svg class="w-7 h-7 text-[#123D2E]" viewBox="0 0 24 24" fill="none" aria-hidden="true">{!! $themeIcons[$program->slug] !!}</svg>
            </div>
            <h3 class="font-display text-xl font-semibold text-[#123D2E] mt-4">{{ localized($program, 'title') }}</h3>
            <div class="w-8 h-[2px] bg-[#C99A3E] mx-auto mt-2 mb-3"></div>
            <p class="text-sm text-[#6F6759] leading-relaxed">{{ localized($program, 'summary') }}</p>
        </div>
    </a>
@endif
