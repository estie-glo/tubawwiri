    ...
    <section class="max-w-7xl mx-auto px-4 py-20 reveal">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 mb-16">
            @forelse ($photos as $photo)
                <img src="{{ asset('storage/' . $photo->file_path) }}" alt="{{ $photo->title }}" class="aspect-square object-cover hover-lift">
            @empty
        ...
        <p class="text-xs font-bold text-[#6B2A28] uppercase tracking-widest mb-6">{{ __('pages.press_releases') }}</p>
        <ul class="space-y-2">
